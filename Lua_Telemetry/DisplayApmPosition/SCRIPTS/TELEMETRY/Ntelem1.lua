...
--
-- Copyright (C) 2015 Luis Vale Gonçalves, Michael Wolkstein, Jochen Kielkopf
--
...
----------------------------------------------------------------------------------
local function playSound(Type, Value, Value2)
	if play_sounds == 1 then
	  if Type == "txt" then
	    if Value == "armed" then
	      playFile(_directory.sounds.."SARM.wav")
	    elseif Value == "disarmed" then
	      playFile(_directory.sounds.."SDISAR.wav")
	    elseif Value == "maxWhReached" then
	      playFile(_directory.sounds.."ALARM3K.wav")
	    end
	  elseif Type == "FM" then
	    playFile(_directory.sounds.."FMOD"..Value..".wav") -- old: AVFM"..(FmodeNr-1).."A.wav
	  elseif Type == "APM_STATUS" then
	    playFile(_directory.sounds.."SEV"..Value..".wav")
	    playFile(_directory.sounds.."MSG"..Value2..".wav") -- old: MSG"..apm_status_message.textnr..".wav
	  end
	end
end
...
local function getTelemetry()
	...
	data.c2          = getValue("Z2")			-- 2. Cell
	data.c3          = getValue("Z3")			-- 3. Cell
	--data.c4          = getValue("Z4")			-- 4. Cell
	--data.c5          = getValue("Z5")			-- 5. Cell
	--data.c6          = getValue("Z6")			-- 6. Cell
	--data.c7          = getValue("Z7")			-- 7. Cell
	...
	--data.lon        = math.rad(getValue("longitude"))	-- current position Longitude
	...
	data.gps_altmax  = getValue("GAlt+")			-- max. GPS Alt
...
local function gpspanel()
	...
	if home.lat~=0 and data.lat~=0 and home.long~=0 and data.lon~=0 then
	  z1 = math.sin(data.lon - home.long) * math.cos(data.lat)
	  z2 = math.cos(home.lat) * math.sin(data.lat) - math.sin(home.lat) * math.cos(data.lat) * math.cos(data.lon - home.long)
